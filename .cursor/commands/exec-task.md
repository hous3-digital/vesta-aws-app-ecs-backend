...
Execute one task of a feature from `tasks/prd-{slug}/{n}_task.md`, keeping Track in sync the way `~/hous3/AGENTS.md` describes (that file, outside this repo, owns the tracker workflow; this command only follows it). Argument: the task file, or the Track task id.
...
4. Write the plan in five lines at most: files, rules and skills that apply, tests per layer, change class, deploy-checklist rows. Then start; do not wait for approval unless the plan changes a `/public/*` contract or the legacy map.
...
5. Mark the task as `completed` in `tasks/prd-{slug}/tasks.md` in a `docs:` commit with the same footer.
