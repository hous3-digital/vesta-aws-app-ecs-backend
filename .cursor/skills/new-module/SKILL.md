...
1. **Schema.** Add the model to `src/infra/database/@prisma/schema.prisma` with `@map` `snake_case` columns and a status enum when the aggregate has a lifecycle. The migration follows the `prisma-migration` skill (`yarn prisma:migrate:local --create-only --name add_{module}`, review the SQL, apply). Ids are TypeID strings with the module name as prefix.
