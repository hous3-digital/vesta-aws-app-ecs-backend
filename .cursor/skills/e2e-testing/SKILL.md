...
6. **Tenant isolation** cases create the resource with issuer A and read it with issuer B: assert 404 (never 403, so existence does not leak: `standard-security` § Tenant isolation) and an empty list. This is the only layer that proves it.
