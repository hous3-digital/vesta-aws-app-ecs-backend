...
3. **Postgres is a projection.** On-chain state (attestations, revocations, registry, commissions) is the source of truth. Tables that mirror it are read models fed by events or reconciliation, never the other way around. Off-chain-only data (KYC status, passkeys, API keys, sessions) is owned by Postgres.
