...
└── tasks/            # specs per feature (prd, tech spec, tasks), written by the commands
```
...
1. **English everywhere in code**: identifiers, comments, JSDoc, Swagger descriptions, commit messages, PR titles. Documentation under `app/docs/` and the specs under `tasks/` may be in Portuguese.
...
| Skills   | `.cursor/skills/*/SKILL.md` | Procedures (create a module, write a test in each layer, add a chain gateway, migrate the schema). Each skill opens by citing the rules it applies                                                                                                                                                                                                                   |
| Commands | `.cursor/commands/*.md`     | Spec-driven flow: `/create-prd` (what and why), `/create-tech-spec` (how), `/create-task` (Track tasks plus one file each, after the user approves the list), `/exec-task` (executes one task keeping Track in sync). Templates in `app/.templates/`. The tracker workflow itself lives in the workspace `AGENTS.md`, outside this repo; the commands only follow it |
...
| Specs    | `tasks/prd-{feature}/`      | `prd.md`, `techspec.md`, `tasks.md` and `{n}_task.md`, produced by the commands; each task file carries its Track task id. Committed with the feature branch                                                                                                                                                                                                         |
