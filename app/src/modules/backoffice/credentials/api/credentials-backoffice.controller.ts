...
} from "@src/modules/backoffice/credentials/application/handlers/credential-list.handler";
...
export class CredentialsBackofficeController {
  ...
  @Get("/")
  public async list(@Query() input: CredentialListInput): Promise<CredentialListResult> {
    const query = new CredentialListQuery(input.status, input.limit ?? 20, input.cursor);
  ...
  @Get("/:id")
  public async detail(@Param("id") id: string): Promise<CredentialDetailResult> {
    return this.queryBus.execute<CredentialDetailQuery, CredentialDetailResult>(new CredentialDetailQuery(id));
  }
}
