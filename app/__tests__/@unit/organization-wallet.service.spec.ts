...
describe("WalletService organization wallet", () => {
  it("uses an issuer-only Privy identity and persists the public Stellar address", async () => {
    ...
    const saved = {
      issuerId: "issuer_a", stellarAddress: "GORGANIZATION", network: "testnet", status: "ACTIVE",
      accountActivated: true, trustlineReady: false, assetCode: "BRL", assetIssuer: "GASSET",
    ...
    const env = {
      PRIVY_APP_ID: "app", PRIVY_APP_SECRET: "secret", STELLAR_NETWORK: "Test SDF Network ; September 2015",
      STELLAR_PAYOUT_ASSET_CODE: "BRL", STELLAR_PAYOUT_ASSET_ISSUER: "GASSET",
    } as EnvService;
    const issuerRepository = { findByExternalId: jest.fn().mockResolvedValue({ externalId: "issuer_a" }) } as unknown as IIssuerRepository;
    const stellar = { ensureAccountExists: jest.fn().mockResolvedValue(undefined) } as unknown as StellarService;
  ...
  it("returns an already active wallet without creating another Privy user", async () => {
    const existing = {
      issuerId: "issuer_a", stellarAddress: "GEXISTING", network: "testnet", status: "ACTIVE",
      accountActivated: true, trustlineReady: true, assetCode: "XLM", assetIssuer: null,
      lastError: null, updatedAt: new Date(),
    };
    const prisma = { organizationWallet: { findUnique: jest.fn().mockResolvedValue(existing) } } as unknown as PrismaService;
    ...
    await expect(service.provisionForOrganization("issuer_a")).resolves.toEqual(expect.objectContaining({ address: "GEXISTING" }));
  });
});
