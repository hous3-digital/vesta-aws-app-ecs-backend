...
describe("proof issuer propagation", () => {
  it("snapshots the resolved credential issuer in the external submit flow", async () => {
    ...
    const handler = new ProofPublicSubmitHandler(
      { saveOrThrow: save } as unknown as IAttestationRepository,
      {
        findByVcHash: jest.fn().mockResolvedValue({
          id: { value: "credential_1" },
          issuerId: "issuer_a",
          issuerDid: "did:web:legacy",
          kycLevel: "basic",
          isApproved: () => true,
          isExpired: () => false,
        }),
      } as unknown as ICredentialRepository,
      {
        findByExternalId: jest.fn().mockResolvedValue({
          externalId: "issuer_a",
          did: { value: "did:pkh:stellar:testnet:GISSUER" },
        }),
      ...
      {
        submitZkProof: jest.fn().mockResolvedValue(stellarResult),
        getContractId: () => "contract",
      } as unknown as StellarService,
    );

    await handler.execute(
      new ProofPublicSubmitCommand(
        "vc_hash",
        {
          pi_a: ["1", "2", "1"],
          pi_b: [
    ...
    expect(save.mock.calls[0][0].issuerDid).toBe("did:pkh:stellar:testnet:GISSUER");
  });
  ...
  it("carries the issuer snapshot from prepare into submit-signed", async () => {
    ...
    const handler = new ProofPublicSubmitSignedHandler(
      { saveOrThrow: save } as unknown as IAttestationRepository,
      {
        submitWithFeeBump: jest.fn().mockResolvedValue(stellarResult),
        getContractId: () => "contract",
      } as unknown as StellarService,
      {} as WalletService,
